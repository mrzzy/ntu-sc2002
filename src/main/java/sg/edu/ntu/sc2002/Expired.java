...
public class Expired {
    ...
    public void cancelChecker(Branch branch) {
        ...
        }, 0, 60000); // Schedule the task to run every 60 seconds (1 minute)
        return;
    }
    ...
    public void checkExpired(Branch branch){
        ...
        for (Order order : branch.getReadyToPickupList()) {
            ...
            if (currentTimeMillis - orderTimestampMillis >= 300000) { // 300000 milliseconds = 5 minutes
                order.cancel();
                branch.getCancelledOrderList().add(order);
                branch.getReadyToPickupList().remove(order); // Remove the order from readyToPickupList
            }
        }
        return;
    }
