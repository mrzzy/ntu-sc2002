...
/**
 * Implements reading of initial Fast Food Chain state from Excel workbooks in
 * resources.
 */
public class Init {
        private static final Map<String, IRole> ROLES = Map.of(
                        "S", new StaffRole(),
                        "M", new ManagerRole(),
                        "A", new AdminRole(),
                        "C", new CustomerRole());
        ...
        public static Chain initChain() {
                ...
                Map<String, User> staffsByName = staffs.values().stream()
                                .flatMap(Set::stream)
                ...
                return rows.stream()
                                ...
                                .collect(
                                                Collectors.groupingBy(
                                                                ...
                                                                Collectors.mapping(
                                                                                (Row row) -> new User(
                                                                                                row.getCellAsString(1)
                                                                                                                .get(),
                                                                                                row.getCellAsString(0)
                                                                                                                .get(),
                                                                                                row.getCellAsString(5)
                                                                                                                .orElse(""),
                                                                                                row.getCellAsNumber(4)
                                                                                                                .get()
                                                                                                                .intValue(),
                                                                                                Gender.fromCode(
                                                                                                                row.getCellAsString(
        ...
        protected static List<Row> readInitWorkbook(String path) {
                ...
                try {
                        ...
                        List<Row> rows = workbook.getFirstSheet()
                                        .openStream()
